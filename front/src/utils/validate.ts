...
function validateSignup(
  values: UserInfomation & {nickname: string} & {passwordForCheck: string},
) {
  const errors = validateUser(values);
  const signupErrors = {...errors, nickname: '', passwordForCheck: ''};

  if (!values.nickname) {
    signupErrors.nickname = '닉네임을 입력해주세요.';
  }

  if (values.password !== values.passwordForCheck) {
    signupErrors.passwordForCheck = '비밀번호가 일치하지않습니다.';
  }
